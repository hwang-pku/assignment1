...
class MatMulOp(Op):
    ...
    def compute(self, node, input_vals):
        ...
        """TODO: Your code here"""
        assert len(input_vals) == 2
        return (input_vals[0].T if node.matmul_attr_trans_A else input_vals[0]) @ (input_vals[1].T if node.matmul_attr_trans_B else input_vals[1])
    ...
    def gradient(self, node, output_grad):
        ...
        return [MatMulOp.__call__(self, output_grad, node.inputs[1], trans_B=True), MatMulOp.__call__(self, node.inputs[0], output_grad, trans_A=True)]
...
class Executor:
    ...
    def run(self, feed_dict):
        ...
        for node in topo_order:
            try:
                node_to_val_map[node] = node.op.compute(node, [node_to_val_map[input_node] for input_node in node.inputs])
            except:
                pass
        # Collect node values.
        ...
        try:
            ...
            if len(node.inputs) == 2:
                if node.inputs[1] not in node_to_output_grads_list:
                    node_to_output_grads_list[node.inputs[1]]=[]
                node_to_output_grads_list[node.inputs[1]].append(lst[1])
        except:
            pass
